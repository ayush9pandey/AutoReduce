...
class SSM(System):
        ...
        if "mode" in kwargs:
            if kwargs.get("mode") == "accurate":
                ...
                def fun_ode(t, x, params):
                    """Evaluate the lambdified function for numeric Jacobians."""
                    y = fun_l(x, params)
                    return np.array(y)

                jfun = nd.Jacobian(lambda x: fun_ode(0, x, P), **kwargs)
                return jfun(x)
        # store the variable with respect to
        # which we approximate the differentiation (df/dvar)
        X = var
        for i in range(self.n):
            for j, _ in enumerate(var):
                ...
                if h != 0:
                    ...
                    var[j] = X[j] + 2 * h
                    f = self.evaluate(
                        fun, var, P, u, set_params_as=set_params_as
                    )
                    F[0] = f[i]
                    var[j] = X[j] + h
                    f = self.evaluate(
    ...
    def compute_SSM(self, normalize=False, **kwargs):
        ...
        def sens_func(t, x, J, Z):
            """Evaluate the sensitivity ODE for a fixed Jacobian and Z term."""
        ...
        for k, _ in enumerate(self.timepoints):
            ...
            # Solve for S = dx/dp for all x and all P
            # (or theta, the parameters) at time point k
            for j in range(len(P)):
                ...
                def sens_func_ode(t, x):
                    """Evaluate the sensitivity ODE closure passed to odeint."""
        ...
        for j, _ in enumerate(self.params_values):
            for i in range(self.n):
                SSM_normalized[:, j, i] = np.divide(
                    self.SSM[:, j, i] * self.params_values[j], self.xs[:, i]
                )
        self.SSM_normalized = SSM_normalized
        return SSM_normalized

    def get_system(self):
        """Return the sensitivity solver state as a base `System`."""
        return System(
            self.x,
            self.f,
            params=self.params,
    ...
    def sensitivity_to_parameter(self, x, j, **kwargs):
        ...
        keyword argument options?:
            ...
            t_min - Starting time.
            t_max - Ending time.
        returns: An OdeSolution object representing the
        (continously-interpolated)
                    sensitivity of each variable to the specified
                    parameter over time.
        """
        return None
        ...
        #     df_dp = lambda xs: nd.Jacobian(ode_as_parameter_call)\
        # (xs).transpose()[:,0]
    ...
    def solve_extended_ode(
        ...
        where S_j is the vector of sensitivities of xs
        to parameter j, f_j is the
        ...
        params:
            ...
            params - a vector of parameters around which to
        ...
        Returns: (x_sols, sensitivities)
            x_sols - An OdeSolution object with the
        ...
        # Solve ODE.
        def ode_func(t, xs):
            """Evaluate the original ODE with fixed parameter values."""
        ...
        if sol.status != 0:
            raise ValueError(
                "In solve_extended_ode, solve_ivp failed with "
                " error message: " + sol.message
            )

        sensitivities = [None] * n_params
        for p in range(n_params):
            print(f"Solving sensitivity for parameter {p + 1}, {n_params}")
            # sensitivities[p] = self.sensitivity_to_parameter(
            #     sol.sol, ode_jac, ode, params, n_variables, p, t_min, t_max
            # )
