...
def test_reduced_models(system, reducible_system):
    ...
    for attempt in possible_reductions:
        ...
        try:
            solve_timescale_test(system, reducible_system, attempt_states = [A, B], 
                                 mode = 'fail',
                                 answer = answer_AB)
        except:
            solve_timescale_test(system, reducible_system, attempt_states = [A, B],
        ...
        solve_timescale_test(system, reducible_system, attempt_states = [A, D], 
                             mode = 'fail',
                             answer = [0, 0])
        solve_timescale_test(system, reducible_system, attempt_states = [A, C], 
                             mode = 'fail',
                             answer = [0, -C*k3])
        solve_timescale_test(system, reducible_system, attempt_states = [B, C, D], 
                             mode = 'success', 
                             answer = [0, -k3 * C, k3 * C])
        solve_timescale_test(system, reducible_system, attempt_states = [A, C, D], 
                             mode = 'success', 
                             answer = [0, -k3 * C, k3 * C])
...
#                                        -k1 * A**2 * B, 
#                                        k1 * A**2 * B]) 
        solve_timescale_test(system, reducible_system, attempt_states = [C, D], 
                             mode = 'success', 
                             answer = [-k3 * C, k3 * C])
        ...
        answer_ABD_eq = [-A**2*B*k1*k3/(k2 + k3), 
                            ...
                            A**2*B*k1*k3/(k2 + k3)]
        try:
            solve_timescale_test(system, reducible_system, attempt_states = [A, B, D], 
                                 mode = 'success',
                                 answer = answer_ABD)
        except:
            solve_timescale_test(system, reducible_system, attempt_states = [A, B, D], 
...
def solve_timescale_test(system, reducible_system,
                         attempt_states = None,
                         mode = None,
                         answer = None):
    if mode == 'fail':
        with pytest.warns(Warning, 
                          match='Solve time-scale separation failed'):
            reduced_system, collapsed_system = get_reduced_model_helper(system, reducible_system, x_hat=attempt_states)
            if reduced_system is not None:
                print(reduced_system.f)
                assert reduced_system.f == answer
                assert collapsed_system.x == [x for x in system.x if x not in attempt_states]
    elif mode == 'success':
        reduced_system, collapsed_system = get_reduced_model_helper(system, reducible_system, x_hat = attempt_states)
